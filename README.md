...
Ada 2 cara mudah untuk mengaktifkan GitHub Pages:

#### Cara 1: Menggunakan Perintah `npm run deploy` (Paling Cepat & Stabil)
1. Buka terminal proyek Anda di komputer.
2. Jalankan perintah:
   ```bash
   npm run deploy
   ```
   *Perintah ini otomatis membangun (`npm run build`) dan mengunggah folder `dist` ke cabang `gh-pages` di GitHub Anda.*
...
6. Website Anda aktif di: `https://<username>.github.io/<nama-repo>/`.

#### Cara 2: Deploy Otomatis via Vercel (Sangat Direkomendasikan)
Vercel adalah platform hosting tercepat dan paling mulus untuk aplikasi React/Vite:
1. Hubungkan akun GitHub Anda ke [Vercel](https://vercel.com).
2. Klik **Add New...** > **Project** > pilih repositori ini.
3. Vercel akan otomatis membaca file `vercel.json` dan melakukan build dengan sempurna.
4. Website Anda langsung online di `https://<nama-proyek>.vercel.app`.
